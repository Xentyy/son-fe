    ...
    <div className="flex items-center justify-center min-h-screen bg-light-bg">
      <div className="p-8 bg-white rounded-2xl shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} className="w-full bg-positive text-white p-3 rounded-lg font-bold hover:bg-green-700 transition disabled:bg-gray-400">
            {loading ? 'Kayıt Olunuyor...' : 'Kayıt Ol'}
          </button>
        </form>
        <p className="text-center mt-6 text-gray-600">
          Zaten bir hesabın var mı? <Link to="/login" className="text-primary font-semibold hover:underline">Giriş Yap</Link>
        </p>
      </div>
    </div>
  );
