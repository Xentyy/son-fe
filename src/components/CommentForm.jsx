...
const CommentForm = ({ bookId, onCommentAdded }) => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="text-center p-6 border-2 border-dashed rounded-lg bg-gray-50">
        <p className="text-gray-700">Yorum yapmak için <Link to="/login" className="font-bold text-primary hover:underline">giriş yapmanız</Link> gerekmektedir.</p>
      </div>
    );
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createCommentForBook(bookId, { text, rating });
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <textarea
        ...
        className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary transition"
        rows="4"
        required
      />
      <button 
        type="submit"
        disabled={submitting}
        className="mt-4 px-6 py-2 bg-primary text-white font-semibold rounded-md hover:bg-accent disabled:bg-gray-400 transition-colors"
      >
